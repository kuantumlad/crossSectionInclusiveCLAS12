...
cout << endl;
 ...
 int nentries = 5000; //anaTree->GetEntriesFast();
...
void select_electron(int run){
  ...
  std::cout << " in select electron " << std::endl;
  ...
  for(Int_t i = 0; i < Npart; i++){
	...
	std::cout << " status " << part_status[i] << std::endl;
        ...
        if( FD_eid_charge_check[i] ) { //FD_eid_default_PID_check[i] ){ // && FD_eid_charge_check[i] && FD_eid_EC_outer_vs_EC_inner_check[i] ){ // && FD_eid_EC_sampling_fraction_check[i] && FD_eid_EC_hit_position_fiducial_check[i]
