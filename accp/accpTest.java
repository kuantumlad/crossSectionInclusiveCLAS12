...
public class accpTest{
	...
	LorentzVector lv_beam = new LorentzVector(0,0,7.546,7.546);
	...
	// create acceptance plots for the inclusive cross section
	// binned in W and Q2
	int w_bins = 40;
	double w_min = 1.1;
	double w_max = 2.1;
	H1F h_acc_q2int_w = new H1F("h_acc_q2int_w","h_acc_q2int_w",w_bins,w_min,w_max);
	H1F h_rec_q2int_w = new H1F("h_rec_q2int_w","h_rec_q2int_w",w_bins,w_min,w_max);
	H1F h_gen_q2int_w = new H1F("h_gen_q2int_w","h_gen_q2int_w",w_bins,w_min,w_max);

	Vector<Double> q2_bins = new Vector<Double>();
	q2_bins.add(0.000);
	q2_bins.add(1.700);
	q2_bins.add(1.950);
	q2_bins.add(2.200);
	...
	Vector<H1F> h_accp_q2bins_w = new Vector<H1F>();
	Vector<H1F> h_rec_q2bins_w = new Vector<H1F>();
	Vector<H1F> h_gen_q2bins_w = new Vector<H1F>();

	for( int b = 0; b < q2_bins.size()-1; b++ ){
	    System.out.println("create histogram for q2 bins " + Double.toString(q2_bins.get(b)) +  " " + Double.toString(q2_bins.get(b+1)) );
	    h_accp_q2bins_w.add( new H1F("h_accp_q2_b"+Integer.toString(b)+"_b"+Integer.toString(b+1)+"w","h_accp_q2_b"+Integer.toString(b)+"_b"+Integer.toString(b+1)+"w",w_bins,w_min,w_max) );

	    h_rec_q2bins_w.add( new H1F("h_rec_q2_b"+Integer.toString(b)+"_b"+Integer.toString(b+1)+"w","h_rec_q2_b"+Integer.toString(b)+"_b"+Integer.toString(b+1)+"w", w_bins,w_min,w_max) );
	    h_gen_q2bins_w.add( new H1F("h_gen_q2_b"+Integer.toString(b)+"_b"+Integer.toString(b+1)+"w","h_gen_q2_b"+Integer.toString(b)+"_b"+Integer.toString(b+1)+"w", w_bins,w_min,w_max) );
	}
	...
	while( file_counter < file_counter_limit ){
		...
		if (event.hasBank("REC::Particle")  == true && event.hasBank("REC::Calorimeter") == true&&event.hasBank("REC::Event")){     
			...
			q.add(lv_beam);
			q.sub(lv_el_rec);
			double q2 = 4.0*lv_beam.e()*lv_el_rec.e() * Math.sin( lv_el_rec.theta()/2.0) * Math.sin( lv_el_rec.theta()/2.0); 
			...
			if (  w > 0.99 ){
			    ...
			    h_rec_q2int_w.fill(w);

			    int q2_bin = -1;
			    for( int q2b = 0; q2b < q2_bins.size()-1; q2b++ ){
				double min=q2_bins.get(q2b);
				double max=q2_bins.get(q2b+1);
				if( q2 >= min && q2 < max ){
				    //System.out.println(" q2 bin min " + Double.toString(min) + " max " + Double.toString(max) + " q2 value is " + Double.toString(q2) );
				    q2_bin = q2b;
				    if( q2_bin >= 0 ){
					//System.out.println(" filling w histo in q2 bin " + Integer.toString(q2b) );
					h_rec_q2bins_w.get(q2_bin).fill(w);
				    }
				}
			    }		    
			}
		...
		if( mcBank_pres ){
			...
			if( rec_pid == electron_pid ){
			    ...
			    q.add(lv_beam);
			    q.sub(lv_el_gen);
			    double q2 = 4.0*lv_beam.e()*lv_el_gen.e() * Math.sin( lv_el_gen.theta()/2.0) * Math.sin( lv_el_gen.theta()/2.0); 
				...
				h_gen_q2int_w.fill(w);
				int q2_bin = -1;
				//System.out.println(" q2 value is " + Double.toString(q2));
				for( int q2b = 0; q2b < q2_bins.size()-1; q2b++ ){
				    double min=q2_bins.get(q2b);
				    double max=q2_bins.get(q2b+1);
				    if( q2 >= min && q2 < max ){
					...
					if( q2_bin >= 0 ){
					    //System.out.println(" filling w histo in q2 bin " + Integer.toString(q2b) );
					    h_gen_q2bins_w.get(q2_bin).fill(w);
					}
				    }
				}			    
			    }
	...
	c_temp1.cd(0);
	c_temp1.getPad(0).getAxisZ().setLog(true);
	...
	c_temp2.cd(0);
	c_temp2.getPad(0).getAxisZ().setLog(true);	
	...
	for( int s = 0; s < 6; s++ ){
	    c_temp5.cd(s);
	    c_temp5.getPad(s).getAxisZ().setLog(true);
	...
	c_temp6.cd(0);
	c_temp6.getPad(0).getAxisZ().setLog(true);
	...
	c_temp7.cd(0);
	c_temp7.getPad(0).getAxisZ().setLog(true);
	...
	c_temp8.draw(h_rec_q2int_w);	    
	c_temp8.save("h_rec_q2int_w.png");

	EmbeddedCanvas c_temp9 = new EmbeddedCanvas();
	c_temp9.setSize(900,900);
	c_temp9.draw(h_gen_q2int_w);	    
	c_temp9.save("h_gen_q2int_w.png");

	EmbeddedCanvas c_temp10 = new EmbeddedCanvas();
	c_temp10.setSize(900,900);
	c_temp10.divide(3,4);
	for( int bb = 0; bb<h_rec_q2bins_w.size(); bb++){
	    c_temp10.cd(bb);
	    c_temp10.draw(h_rec_q2bins_w.get(bb));	    
	}
	c_temp10.save("h_rec_q2bins_w.png");

	EmbeddedCanvas c_temp11 = new EmbeddedCanvas();
	c_temp11.setSize(900,900);
	c_temp11.divide(3,4);
	for( int bb = 0; bb<h_gen_q2bins_w.size(); bb++){
	    c_temp11.cd(bb);
	    c_temp11.draw(h_gen_q2bins_w.get(bb));	    
	}
	c_temp11.save("h_gen_q2bins_w.png");

	////////////////////////////////////////////////////////////////////////////////////////////
	// cal the acceptance for inclusive cross section then print to txt file
	EmbeddedCanvas c_temp12 = new EmbeddedCanvas();
	c_temp12.setSize(900,900);
	c_temp12.divide(3,4);
	Vector<GraphErrors> g_w_accp_per_q2 = new Vector< GraphErrors>(); 
	for( int qb = 0; qb < h_gen_q2bins_w.size(); qb++ ){
	    
	    //get rec and gen histo
	    H1F h_rec = h_rec_q2bins_w.get(qb);
	    H1F h_gen = h_gen_q2bins_w.get(qb);

	    int nbins = h_rec.getAxis().getNBins();
	    
	    // loop over rec ad gen W bins to calc accp
	    double[] w_bin_center;
	    w_bin_center = new double[nbins];
	    
	    double[] w_bin_center_err;
	    w_bin_center_err = new double[nbins];  
	    
	    double[] accp;
	    accp  =  new double[nbins]; 
	    double[] accp_err ;
	    accp_err =  new double[nbins];   

	    for( int bb = 0; bb < nbins; bb++ ){		
		double bin_center = h_rec.getAxis().getBinCenter(bb);
		double rec_content = h_rec.getBinContent(bb);
		double gen_content = h_gen.getBinContent(bb);

		double ratio_bin_content = h_rec.getBinContent(bb)/h_gen.getBinContent(bb);
		System.out.println(" bin center " + Double.toString(bin_center) + " rec " + Double.toString(h_rec.getBinContent(bb))+ " gen " + Double.toString(h_gen.getBinContent(bb)) );

		w_bin_center[bb] = bin_center;
		w_bin_center_err[bb] = 0.0;
	       
		accp[bb]=ratio_bin_content;
		if ( gen_content == 0 || gen_content < 1 ) accp[bb]=0.0;

		accp_err[bb] = 0.0;

	    }

	    g_w_accp_per_q2.add( new GraphErrors("accp_w_q2bin_"+Integer.toString(qb), w_bin_center,  accp, w_bin_center_err, accp_err) );  				   
	}

	EmbeddedCanvas c_temp12a = new EmbeddedCanvas();                                                                                                                                                   
        c_temp12a.setSize(10000,9000); 
	c_temp12a.divide(3,4);
	for( int qb = 0 ; qb < h_gen_q2bins_w.size(); qb++ ){  
	    c_temp12a.cd(qb);
	    c_temp12a.draw(g_w_accp_per_q2.get(qb));
	}
	c_temp12a.save("g_w_accp_per_q2_bin.png");
